...
public class CocktailWidget extends HUDWidget {
    ...
    public static final String IDENTIFIER = "MineboxAddons:CocktailWidget";

    public CocktailWidget() {
        super(IDENTIFIER);
        if(MineboxAddonsClient.CONFIG.cocktailTimerX() == -1337 && MineboxAddonsClient.CONFIG.cocktailTimerY() == -1337) {
            setPosition(0, 0);
        } else {
            setPosition(MineboxAddonsClient.CONFIG.cocktailTimerX(), MineboxAddonsClient.CONFIG.cocktailTimerY());
        }
    }

    @Override
    protected void savePosToConfig(int xPos, int yPos) {
        MineboxAddonsClient.CONFIG.cocktailTimerY(xPos);
        MineboxAddonsClient.CONFIG.cocktailTimerY(yPos);
        MineboxAddonsClient.CONFIG.save();
    }

    @Override
    public void render(DrawContext context, float delta) {
        if(!MineboxAddonsClient.CONFIG.cocktailTimer()) return;
        // Update text and calculate base dimensions
        currentText = "Cocktail in: " + timeTo + " minutes";
